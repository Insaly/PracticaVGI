...
// Selecció del model d'iluminació segons variable ilumin
    ...
    case PLANA:
		...
		// PRÀCTICA FRACTAL: Crida a la funció pel càlcul dels vectors normals a les cares si l'objecte és un fractal
	...
	case SUAU:
		...
		// Il.luminació per cares suavitzat
...
// Vista_Ortogràfica: Ilumina i dibuixa l'escena. Crida a la funció glm::LookAt segons la variable prj 
//				(planta, alçat, perfil o axonometrica).
glm::mat4 Vista_Ortografica(GLuint sh_programID, int prj,GLdouble Raux,CColor col_fons,CColor col_object,
				char objecte,GLdouble mida,int step,
...
glm::mat4 Projeccio_Perspectiva(GLuint sh_programID, int minx, int miny, GLsizei w, GLsizei h, double zoom)
{
	...
	double fovH = glm::radians(60.0); 
	double fovV;

	if (aspect < 1.0)
	{
		fovV = 2.0 * atan(tan(fovH * 0.5) / aspect);
	}
	else
	{
		fovV = fovH;
	...
	// Projecció
	MatriuProjeccio = glm::perspective(fovV, aspect, p_near, p_far);

	// Passem matriu al shader
	glUniformMatrix4fv( glGetUniformLocation(sh_programID, "projectionMatrix"), 1, GL_FALSE, &MatriuProjeccio[0][0]);
				 ...
				 bool eix, CMask3D reixa, CPunt3D hreixa)
{
	GLdouble cam[3],up[3];
	glm::mat4 MatriuVista(1.0);
	glm::mat4 TransMatrix(1.0);		// Matriu Traslació
...
// Posició càmera i vector cap amunt segons posició dels eixos.
...
// Especificació de la càmera i paràmetres
   MatriuVista = glm::lookAt(
...
//Vista_Navega: Definició glm::lookAt directament per paràmetre, sense esfèriques.
...
//Vista_Geode: Definició glm::lookAt() a partir de coordenades esfèriques, però situant la càmera a l'origen (0,0,0)
...
// deixos: Creació del VAO per al dibuix dels eixos coordenats
GLuint deixos()
{
...
/* Codi OpenGL 1.1 per al dibuix dels eixos.
// Eix X (vermell)
...
// draw_Eixos: Dibuix del VAO dels eixos coordenats
...
// Grids drawing
	if (quadricula.w) draw_LinVAO_Object(GRID_XYZ);			// gridXY(GRID_SIZE);
	else  {	if (quadricula.x) draw_LinVAO_Object(GRID_XY);	// gridXY(GRID_SIZE, hquadricula.z);
			if (quadricula.y) draw_LinVAO_Object(GRID_XZ);	// gridXZ(GRID_SIZE, hquadricula.y);
			if (quadricula.z) draw_LinVAO_Object(GRID_YZ);	// gridYZ(GRID_SIZE, hquadricula.x);
			}
...
// loadGridXY: Construeix un grid XY de tamany size amb pas 1 situat una alçada h de l'eiz Z.
...
// VAO, VBO
	CVAO VAOGrid;
	VAOGrid.vaoId = 0;	VAOGrid.vboId = 0; VAOGrid.eboId = 0; VAOGrid.nVertexs = 0;	VAOGrid.nIndices = 0;
...
// Eix X
	...
	for (i = -size; i < size + 1; i++)
		{	//glBegin(GL_LINES);
			  colors.push_back(1.0);		colors.push_back(0.0);			colors.push_back(0.0);	colors.push_back(0.5);
...
// ----------------------- VBO
	...
	VAOGrid = load_LINES_VAO(vertices, colors);

	return VAOGrid;
}

// loadGridXZ: Construeix un VAO amb el grid YZ de tamany size amb pas 1 situat una alçada h de l'eiz Z.
...
// Eix Z
	...
	for (i = -size; i < size + 1; i++)
		{	//glBegin(GL_LINES);
			  colors.push_back(0.0);		colors.push_back(1.0);	colors.push_back(1.0); colors.push_back(0.5);
			  //glVertex3f(-size, h, i);
...
// loadGridYZ: Construeix un VAO amb el grid YZ de tamany size amb pas 1 situat una alçada h de l'eiz Z.
...
// Eix Y
	...
	for (i = -size; i < size + 1; i++)
		{	//glBegin(GL_LINES);
		      colors.push_back(0.0);		colors.push_back(1.0);			colors.push_back(0.0);	colors.push_back(0.5);
...
// loadGridXYZ: Construeix un VAO amb el grid 3D de tamany size amb pas 1.
