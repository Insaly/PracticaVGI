...
// Definició propietats de reflexió (emissió, ambient, difusa, especular) del material.
	...
	case PAISATGE:
		...
		glUniformMatrix4fv(
			glGetUniformLocation(sh_programID, "modelMatrix"),
			1, GL_FALSE, &ModelMatrix[0][0]
		);

		NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
		glUniformMatrix4fv(
			glGetUniformLocation(sh_programID, "normalMatrix"),
...
// Dibuix de l'objecte TIE (Nau enemiga Star Wars)
	case TIE:
		...
		// Transformaciones PREVIAS a la trayectoria
		ModelMatrix = glm::translate(MatriuTG, glm::vec3(0.0f, -10.0f, 0.0f));
...
void faro(GLuint shaderID, glm::mat4 MatriuVista, glm::mat4 MatriuTG, bool sw_mat[5])
{
	...
	glUniform4f(
		glGetUniformLocation(shaderID, "LightSource[7].position"),
		0.0f, 0.0f, 24.0f, 1.0f
	);

	glUniform4f(
		glGetUniformLocation(shaderID, "LightSource[7].diffuse"),
		1.0f, 1.0f, 1.0f, 1.0f
	);

	glUniform4f(
		glGetUniformLocation(shaderID, "LightSource[7].specular"),
		1.0f, 1.0f, 1.0f, 1.0f
	);

	glUniform4f(
		glGetUniformLocation(shaderID, "LightSource[7].spotDirection"),
		0.0f, -1.0f, -0.5f, 1.0f
	);
	glUniform1f(
		glGetUniformLocation(shaderID, "LightSource[7].spotCosCutoff"),
		0.707106f
	);

	glUniform1f(
		glGetUniformLocation(shaderID, "LightSource[7].spotExponent"),
		5.0f
	);
